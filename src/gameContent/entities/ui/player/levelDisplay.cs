...
class LevelDisplay : Entity {
    Renderer renderer;
    SpriteFont font;
    int currentResource;

    public LevelDisplay(Game _game) : base(_game) {
        currentResource = 0;
        renderer = _game.Services.GetService<Renderer>();
        font = _game.Content.Load<SpriteFont>("fonts/MainFont");
        SetScale(3);
        SetSprite("round_icon", LayerType.UI);

        // init hover msg
        var hover = new HoverMessageComponent(_game, this);
        hover.SetMessage("Round", "Current round you have reached");

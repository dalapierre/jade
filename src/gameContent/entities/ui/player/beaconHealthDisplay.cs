...
class BeaconHealthDisplay : Entity {
    Renderer renderer;
    SpriteFont font;
    int currentResource;
    
    public BeaconHealthDisplay(Game _game) : base(_game) {
        currentResource = 0;
        renderer = _game.Services.GetService<Renderer>();
        font = _game.Content.Load<SpriteFont>("fonts/MainFont");
        SetScale(3);
        SetSprite("beacon_health", LayerType.UI);

        // init hover msg
        var hover = new HoverMessageComponent(_game, this);
        hover.SetMessage("Beacon Health", "Health of the enemy beacon", "You win once this reaches zero and lose", "if you lose all your units before then");
    ...
    public override void Draw() {
        ...
        var yy = position.Y + GetComponent<Sprite>().height / 3.5f;
        var size = 14;
